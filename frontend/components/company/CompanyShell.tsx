...
const navItems = [
  { label: "Dashboard",     href: "/company",               icon: <DashboardIcon /> },
  { label: "Profile",       href: "/company/profile",       icon: <PersonIcon /> },
  { label: "New JNF",       href: "/company/jnf/new",       icon: <WorkIcon /> },
  { label: "New INF",       href: "/company/inf/new",       icon: <DescriptionIcon /> },
  { label: "Submissions",   href: "/company/submissions",   icon: <ViewListIcon /> },
  { label: "Notifications", href: "/company/notifications", icon: <NotificationsIcon /> },
];
...
export default function CompanyShell({ children }: { children: React.ReactNode }) {
  ...
  const currentNav = navItems.find((item) =>
    item.href === "/company" ? pathname === "/company" : pathname.startsWith(item.href)
  );
  const pageTitle = currentNav ? currentNav.label : "Company Portal";

  // Sidebar content (used in both mobile and desktop drawers)
  const drawerContent = (
      ...
      >
        <Toolbar sx={{ px: { xs: 2, sm: 3, md: 4 }, minHeight: { xs: 60, md: 70 } }}>
          ...
          <Stack direction="row" spacing={2} alignItems="center">
            {/* You could add a quick notification bell icon here if desired */}
